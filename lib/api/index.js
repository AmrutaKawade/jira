...
events.on('github-event', async ({ event, payload }) => {
  ...
  console.log(('logging githubHost'));
  console.log((githubHost));
  const ghaeInstanceData = await getInstanceMetadata(githubHost);

  console.log(ghaeInstanceData.appId);
  console.log(ghaeInstanceData.privateKey);

  console.log('logging payload');
  const github = createAppAuth({
    id: ghaeInstanceData.appId,
    privateKey: ghaeInstanceData.privateKey,
    installationId: payload.installation.id,
  });

  const octokit = GitHubAPI({baseUrl: process.env.GHE_HOST && (process.env.GHE_PROTOCOL || 'https') + '://' + process.env.GHE_HOST + '/api/v3',});
  const context = { github, payload , octokit };
...
app.post('/github/events', (req, res) => {
  ...
  const isValidWebhook = webhooks.verify(req.body, req.get('X-Hub-Signature-256'));
