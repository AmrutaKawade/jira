...
const { ctx } = require('probot').Context;
...
module.exports = (robot, { getRouter }) => {
  ...
  app.post('/github/events', async (req, res) => {
    ...
    if (isValidWebhook) {
      ...
      console.log('eventNameWithAction');
      console.log(eventNameWithAction);
      ...
      client.trackTrace('testing insights Event received', appInsights.Contracts.SeverityLevel.Information);
      // let customProp = { event: context.name, action: context.payload.action };
      // let msg = JSON.stringify(customProp) + 'testing insights Event received';
      // client.trackTrace(msg, appInsights.Contracts.SeverityLevel.Information, customProp);
