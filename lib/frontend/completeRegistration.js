const url = require('url');
const fetch = require("node-fetch");
const { AppSecrets } = require('../models');
const { getSignedJsonWebToken } = require('../common/helper');
const { getInstanceMetadata } = require('../common/helper');
const { GitHubAPI } = require('../config/github-api');

module.exports = async (req, res, next) => {

    console.log(JSON.stringify(req.url))
    const { query } = url.parse(req.url, true);
    console.log(query.code)

    function getCookie(cookie) {
      var name = "githubHost=";
      var decodedCookie = decodeURIComponent(cookie);
      var ca = decodedCookie.split(';');
      for(var i = 0; i <ca.length; i++) {
        var c = ca[i];
        while (c.charAt(0) == ' ') {
          c = c.substring(1);
        }
        if (c.indexOf(name) == 0) {
          return c.substring(name.length, c.length);
        }
      }
      return "";
    }

    // parse the request and 
    var githubHost = getCookie(req.headers.cookie)
    console.log(githubHost)
    url_fetch = "https://" + githubHost + "/api/v3/app-manifests/"+ query.code +"/conversions"
    var result = undefined

    result = await fetch(url_fetch,{
            method: "POST",
            headers:{
              "accept": "application/vnd.github.v3+json"
          }
          }).then(response => response.json()).then(data => {return data}).catch((error) => {
            console.error('Error:', error);
          });
    
    const secrets = await AppSecrets.insert({
      clientId: result.client_id, 
      clientSecret: result.client_secret, 
      privateKey: result.pem, 
      appId: result.id,
      githubHost: new URL(result.html_url).hostname,
    webhookSecret: result.webhook_secret,
    });

    // Update the webhook secret
    // const hostname = new URL(result.html_url).hostname;
    // const ghaeInstanceData = await getInstanceMetadata(hostname);
    // const jwt = getSignedJsonWebToken({ appId: ghaeInstanceData.appId, privateKey: ghaeInstanceData.privateKey });
    //
    // const octokit = GitHubAPI({
    //     auth: { token: jwt },
    //     baseUrl: `https://${hostname}/api/v3`,
    // });
    //
    // await octokit.request('PATCH /app/hook/config', {
    //     secret: 'development',
    // });
    return res.render("ghae_register_complete.hbs", {
      app: JSON.stringify(result)
    });
  };