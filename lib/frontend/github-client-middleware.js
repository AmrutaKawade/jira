...
const { getInstanceMetadata, getSignedJsonWebToken } = require('../common/helper');
...
module.exports = (getAppToken) => (req, res, next) => {
  ...
  const ghaeInstanceData = getInstanceMetadata(host);

  if (ghaeInstanceData) {
    const privateKey = getPrivateKey({ filepath : ghaeInstanceData.privateKeyPath } );
    res.locals.client = GitHubAPI({
      auth: { token : getSignedJsonWebToken({ appId: ghaeInstanceData.appId, privateKey }) },
      baseUrl: "https://" + host + "/api/v3",
    });
  } else {
    res.locals.client = GitHubAPI();
