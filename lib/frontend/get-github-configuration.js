...
module.exports = async (req, res, next) => {
  ...
  const { github, isAdmin } = res.locals;
  ...
  const host = req.session.githubHost;
  const ghaeInstaneData = getInstanceMetadata(host);
  console.log("GHAE data: " + JSON.stringify(ghaeInstaneData));
  const privateKey = readFileSync(ghaeInstaneData.privateKeyPath, "utf-8");
