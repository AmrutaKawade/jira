...
const showRegistrationPage = require('./showRegistration');
...
module.exports = (appTokenGenerator) => {
  ...
  app.get('/ghaeRegister',showRegistrationPage);  
  app.get('/ghaeRegisterComplete',completeRegistration); 
  ...
  app.get('/github/configuration', csrfProtection, githubClientMiddleware, getGitHubConfiguration);
  app.post('/github/configuration', csrfProtection, githubClientMiddleware, postGitHubConfiguration);

  app.get('/github/installations', csrfProtection, githubClientMiddleware, listGitHubInstallations);
  app.get('/github/subscriptions/:installationId', csrfProtection, githubClientMiddleware, getGitHubSubscriptions);
  app.post('/github/subscription', csrfProtection, deleteGitHubSubscription);

  app.get('/jira/configuration', csrfProtection, verifyJiraMiddleware, githubClientMiddleware, getJiraConfiguration);
  ...
  app.get('/', githubClientMiddleware, async (req, res, next) => {
  ...
  oauth.on('token', (token, res, tokenRes, req) => {
    console.log("#### App.js frontend - " + JSON.stringify(req.session));
