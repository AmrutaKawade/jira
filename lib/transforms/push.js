...
function addComment(jobData) {
  // This code sample uses the 'node-fetch' library:
  // https://www.npmjs.com/package/node-fetch
  
  let comment = `Author: `+jobData.repository.owner.name + `\
  repository: `+jobData.repository.full_name + `\
  commit: `+jobData.commits[0].id + `\
  commiturl: `+jobData.commits[0].url + `\
  commitmessage: `+jobData.commits[0].message;
  ...
  console.log("comment body: " + bodyData);

  let issueIdOrKey = jobData.shas[0].issueKeys;
  console.log("Issue Id Or Key: " + issueIdOrKey);
  fetch(`https://amkgithub.atlassian.net/rest/api/3/issue/${issueIdOrKey}/comment`, {
    method: 'POST',
    headers: {
      'Authorization': `Basic ${Buffer.from(
        'amrutakawade@github.com:VyZ5j18hcfGSWqeBeWeF170E'
      ).toString('base64')}`,
