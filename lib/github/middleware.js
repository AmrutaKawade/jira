...
module.exports = function middleware(callback) {
  return withSentry(async (context) => {
    ...
    const githubAPIURL = extractBaseURL(context.payload);
    const githubHost = new URL(githubAPIURL).hostname;

    const gitHubInstallationId = context.payload.installation.id;
    enhanceOctokit(context.octokit, context.log, { githubHost, gitHubInstallationId } );
    ...
    if (context.payload.sender.type === 'Bot') {
      context.log.error({ noop: 'bot', botId: context.payload.sender.id, botLogin: context.payload.sender.login }, 'Halting futher execution since the sender is a bot');
      return;
    }

    if (isFromIgnoredRepo(context.payload)) {
      context.log.error({ noop: 'ignored_repo', installation_id: context.payload.installation.id, repository_id: context.payload.repository.id }, 'Halting futher execution since the repository is explicilty ignored');
      return;
    }
