...
module.exports.discovery = (app, queues) => async function discoverContent(job) {
  ...
  enhanceOctokit(github, app.log, { githubHost: subscription.githubHost, gitHubInstallationId: installationId });

  const repositories = await github.paginate(github.apps.listReposAccessibleToInstallation({ per_page: 100 }), res => res.data.repositories);
